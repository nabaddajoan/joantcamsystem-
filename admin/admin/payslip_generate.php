<?php
	...
	$sql = "SELECT *, SUM(num_hr) AS total_hr, attendance.teacher_id AS empid, teachers.teacher_id AS employee FROM attendance LEFT JOIN teachers ON teachers.id=attendance.teacher_id LEFT JOIN subject ON subject.id=teachers.position_id WHERE date BETWEEN '$from' AND '$to' GROUP BY attendance.teacher_id ORDER BY teachers.lastname ASC, teachers.firstname ASC";

	$query = $conn->query($sql);
	while($row = $query->fetch_assoc()){
		$empid = $row['empid'];
                      
      	$casql = "SELECT *, SUM(amount) AS cashamount FROM cashadvance WHERE teacher_id='$empid' AND date_advance BETWEEN '$from' AND '$to'";
